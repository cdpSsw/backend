  ...
  async (req, res) => {
    try {
      ...
      const [result] = await conn.query(
        `INSERT INTO ${table} (position, name, tel, email, website, education, expertise, expLocation, expPosition, research, image, image_data, image_mime_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          position,
          name,
          tel,
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      // อัปเดตฐานข้อมูล
      const [result] = await conn.query(
        `UPDATE ${table} SET position = ?, name = ?, tel = ?, email = ?, website = ?, education = ?, expertise = ?, expLocation = ?, expPosition = ?, research = ?, image = ? WHERE id = ?`,
        [
          position,
          name,
          tel,
